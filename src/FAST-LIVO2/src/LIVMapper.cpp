...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &node, image_transport::ImageTransport &it_)
{
  ...
  auto deskew_qos = rclcpp::QoS(rclcpp::KeepLast(1)).best_effort();
...
void LIVMapper::publish_deskew_cloud_body(
    const PointCloudXYZI::Ptr &cloud_body,
    double stamp_time)
{
  if (!pubDeskewCloudBody)
  {
    return;
  }

  // 没有订阅者时不做 pcl::toROSMsg，避免浪费 FAST-LIVO 主线程 CPU
  if (pubDeskewCloudBody->get_subscription_count() == 0)
  {
    return;
  }
  ...
  if (stamp_time <= 0.0)
  {
    RCLCPP_WARN_THROTTLE(
        ...
        *this->node->get_clock(),
        1000,
        "[DESKEW_CLOUD] invalid stamp_time=%.6f",
        stamp_time);
    return;
  }
  ...
  cloud_msg.header.stamp = sec2Stamp(stamp_time);
  cloud_msg.header.frame_id = "body";

  pubDeskewCloudBody->publish(cloud_msg);

  RCLCPP_INFO_THROTTLE(
      this->node->get_logger(),
      *this->node->get_clock(),
      1000,
      "[DESKEW_CLOUD] published cloud_body size=%zu, time=%.6f",
      cloud_body->points.size(),
      stamp_time);
}
