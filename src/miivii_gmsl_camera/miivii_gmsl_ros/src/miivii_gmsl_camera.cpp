...
namespace miivii_gmsl
{
  ...
  void MiiviiGmslCamera::DeclareParameters()
  {
    ...
    declare_parameter<int>("sync_freq", 10);
...
{
    static uint64_t get_ok_count = 0;
    static uint64_t pub_count = 0;
    static uint64_t get_err_count = 0;
    static uint64_t stamp_gap_count = 0;
    static uint64_t zero_stamp_drop_count = 0;
    static uint64_t duplicate_stamp_drop_count = 0;
    static uint64_t last_published_stamp_ns = 0;

    static uint64_t last_get_ok_count = 0;
    static uint64_t last_pub_count = 0;
    static uint64_t last_get_err_count = 0;
    static uint64_t last_stamp_gap_count = 0;

    static int64_t last_stamp_ns = 0;
    static double last_stamp_dt_ms = 0.0;
    static double max_stamp_dt_ms = 0.0;

    static auto stat_last_wall = std::chrono::steady_clock::now();
    ...
    //uint64_t timestamp_from_getimage = timestamp;
    //uint64_t timestamp_from_gmsl = 0;
    //(void)this->mvcam->GetGmslTimeStamp(camera_no, timestamp_from_gmsl);
...
if (!res)
{
  ...
  RCLCPP_ERROR_THROTTLE(
      ...
      *this->get_clock(),
      1000,
      "Get Image Error!! total_err=%llu",
      static_cast<unsigned long long>(get_err_count));
  return;
}

get_ok_count++;

    const uint64_t timestamp_from_getimage = timestamp;
    uint64_t timestamp_from_gmsl = 0;
    const bool got_gmsl_timestamp = this->mvcam->GetGmslTimeStamp(camera_no, timestamp_from_gmsl);

    // 只使用 SDK/GMSL 时间戳作为图像发布时间戳
    // timestamp == 0：直接丢弃
    // timestamp 与上一帧相同：只保留第一帧，后续重复帧丢弃
    uint64_t publish_timestamp = 0;
    if (got_gmsl_timestamp)
    {
      publish_timestamp = timestamp_from_gmsl;
    }
    ...
    {
      duplicate_stamp_drop_count++;
      RCLCPP_WARN_THROTTLE(
          this->get_logger(),
          *this->get_clock(),
          1000,
          "[CAMERA_DROP_DUP_STAMP] drop duplicate timestamp image, "
          "timestamp=%llu, drop_count=%llu",
          static_cast<unsigned long long>(publish_timestamp),
          static_cast<unsigned long long>(duplicate_stamp_drop_count));
      return;
    }

    last_published_stamp_ns = publish_timestamp;

    builtin_interfaces::msg::Time stamp_msg;
    int64_t stamp_ns = static_cast<int64_t>(publish_timestamp);
...
if (this->sync_camera_freq > 0)
{
  expected_hz = static_cast<double>(this->sync_camera_freq);
}
else if (this->publish_rate > 0.0)
{
  expected_hz = this->publish_rate;
}

double expected_dt_ms = 0.0;
if (expected_hz > 0.0)
{
  expected_dt_ms = 1000.0 / expected_hz;
}

if (last_stamp_ns > 0 && stamp_ns > last_stamp_ns)
{
  last_stamp_dt_ms = static_cast<double>(stamp_ns - last_stamp_ns) / 1000000.0;

  if (last_stamp_dt_ms > max_stamp_dt_ms)
  {
    max_stamp_dt_ms = last_stamp_dt_ms;
  }

  if (expected_dt_ms > 1.0 && last_stamp_dt_ms > expected_dt_ms * 1.5)
  {
    int maybe_lost = static_cast<int>((last_stamp_dt_ms / expected_dt_ms) + 0.5) - 1;
    if (maybe_lost < 0)
    {
      maybe_lost = 0;
    }
    ...
    RCLCPP_WARN(
        this->get_logger(),
        "[CAMERA_STAMP_GAP] dt=%.3f ms, expected=%.3f ms, maybe_lost=%d",
        last_stamp_dt_ms,
        expected_dt_ms,
        maybe_lost);
  }
...
/*       std::string encoding;
      ...
      {
        RCLCPP_WARN(this->get_logger(), "Unsupported format combination for camera %d", i);
        continue;
      }
      ...
      sensor_msgs::msg::Image image;
      std::string frame_id = "camera" + std::to_string(ctx[i].dev_node[10] - 0x30);
	...
	if (ctx[i].camera_fmt_str == "UYVY" && ctx[i].output_fmt_str == "UYVY")
	{
	  cv::Mat uyvy_mat(image.height, image.width, CV_8UC2, outbuf[i]);

	  // UYVY / YUV422 直接转 RGB，一次转换完成
	  cv::cvtColor(uyvy_mat, rgb_image, cv::COLOR_YUV2RGB_UYVY);
	}
	else if (ctx[i].camera_fmt_str == "UYVY" && ctx[i].output_fmt_str == "BGRA32")
	{
	  cv::Mat bgra_mat(image.height, image.width, CV_8UC4, outbuf[i]);

	  // BGRA 直接转 RGB，一次转换完成
	  cv::cvtColor(bgra_mat, rgb_image, cv::COLOR_BGRA2RGB);
	}
	else
	{
	  RCLCPP_WARN(this->get_logger(), "Unsupported format combination for camera %d", i);
      ...
      // 填充ROS Image消息字段
      image.encoding = "rgb8";
      image.step = rgb_image.cols * 3;

      // 复制数据
      image.data.assign(rgb_image.datastart, rgb_image.dataend);
    ...
    auto stat_now_wall = std::chrono::steady_clock::now();
    double stat_sec = std::chrono::duration<double>(stat_now_wall - stat_last_wall).count();

    if (stat_sec >= 1.0)
    {
      uint64_t delta_get = get_ok_count - last_get_ok_count;
      uint64_t delta_pub = pub_count - last_pub_count;
      uint64_t delta_err = get_err_count - last_get_err_count;
      uint64_t delta_gap = stamp_gap_count - last_stamp_gap_count;

      int cam_count = this->active_camera_num > 0 ? this->active_camera_num : 1;

      double get_hz = static_cast<double>(delta_get) / stat_sec;
      double pub_total_hz = static_cast<double>(delta_pub) / stat_sec;
      double pub_per_cam_hz = pub_total_hz / static_cast<double>(cam_count);

      RCLCPP_INFO(
          this->get_logger(),
          "[CAMERA_PUB_STAT] get=%.2f Hz, pub_total=%.2f Hz, pub_per_cam=%.2f Hz, err=%llu/s, stamp_gap=%llu/s, stamp_dt=%.3f ms, max_dt=%.3f ms, size=%ux%u",
          get_hz,
          pub_total_hz,
          pub_per_cam_hz,
          static_cast<unsigned long long>(delta_err),
          static_cast<unsigned long long>(delta_gap),
          last_stamp_dt_ms,
          max_stamp_dt_ms,
          ctx[0].out_w,
          ctx[0].out_h);

      last_get_ok_count = get_ok_count;
      last_pub_count = pub_count;
      last_get_err_count = get_err_count;
      last_stamp_gap_count = stamp_gap_count;
      stat_last_wall = stat_now_wall;
      max_stamp_dt_ms = 0.0;
    }
  }
  }
