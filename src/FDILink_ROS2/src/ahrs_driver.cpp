...
void ahrsBringup::processLoop()  // 数据处理过程
{
  ...
  while (rclcpp::ok())
  {
    ...
    {
      RCLCPP_DEBUG_THROTTLE(this->get_logger(), *this->get_clock(), 5000, "skip unsupported fdilink frame: type=0x%02X len=%u", head_type[0], check_len[0]);
      uint8_t skip_buf[512];
      size_t skip_len = static_cast<size_t>(check_len[0]) + 4;
      if (skip_len > sizeof(skip_buf)) skip_len = sizeof(skip_buf);
      serial_.read(skip_buf, skip_len);
      continue;
    }
    if (head_type[0] == TYPE_IMU && check_len[0] != IMU_LEN)
    {
      RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000, "head_len error (imu): got %u, expected %u", check_len[0], IMU_LEN);
      uint8_t skip_buf[512];
      size_t skip_len = static_cast<size_t>(check_len[0]) + 5;
      if (skip_len > sizeof(skip_buf)) skip_len = sizeof(skip_buf);
      serial_.read(skip_buf, skip_len);
      continue;
    }else if (head_type[0] == TYPE_AHRS && check_len[0] != AHRS_LEN)
    {
      RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000, "head_len error (ahrs): got %u, expected %u", check_len[0], AHRS_LEN);
      uint8_t skip_buf[512];
      size_t skip_len = static_cast<size_t>(check_len[0]) + 5;
      if (skip_len > sizeof(skip_buf)) skip_len = sizeof(skip_buf);
      serial_.read(skip_buf, skip_len);
    ...
    {
      RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000, "head_len error (insgps): got %u, expected %u", check_len[0], INSGPS_LEN);
      uint8_t skip_buf[512];
      size_t skip_len = static_cast<size_t>(check_len[0]) + 5;
      if (skip_len > sizeof(skip_buf)) skip_len = sizeof(skip_buf);
      serial_.read(skip_buf, skip_len);
    ...
    {
      RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000, "head_len error (GEODETIC_POS): got %u, expected %u", check_len[0], GEODETIC_POS_LEN);
      uint8_t skip_buf[512];
      size_t skip_len = static_cast<size_t>(check_len[0]) + 5;
      if (skip_len > sizeof(skip_buf)) skip_len = sizeof(skip_buf);
      serial_.read(skip_buf, skip_len);
    ...
    else if (head_type[0] == TYPE_GEODETIC_POS)
    {
      ...
      const float h_acc = Geodetic_Position_frame_.frame.data.data_pack.hAcc;
      const float v_acc = Geodetic_Position_frame_.frame.data.data_pack.vAcc;
      gps_data.status.service = sensor_msgs::msg::NavSatStatus::SERVICE_GPS;
      gps_data.status.status =
          (std::isfinite(gps_data.latitude) && std::isfinite(gps_data.longitude))
              ? sensor_msgs::msg::NavSatStatus::STATUS_FIX
              : sensor_msgs::msg::NavSatStatus::STATUS_NO_FIX;
      if (std::isfinite(h_acc) && h_acc > 0.0f && std::isfinite(v_acc) && v_acc > 0.0f)
      {
        gps_data.position_covariance[0] = h_acc * h_acc;
        gps_data.position_covariance[4] = h_acc * h_acc;
        gps_data.position_covariance[8] = v_acc * v_acc;
        gps_data.position_covariance_type =
