...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  try_declare.template operator()<bool>("gnss_constraint.enable", false);
  try_declare.template operator()<std::string>("gnss_constraint.odom_topic", "/gnss/odom_gated");
  try_declare.template operator()<double>("gnss_constraint.max_age", 0.3);
  try_declare.template operator()<double>("gnss_constraint.gain", 0.05);
  try_declare.template operator()<double>("gnss_constraint.max_correction", 0.05);
  try_declare.template operator()<double>("gnss_constraint.reject_threshold", 2.0);
  try_declare.template operator()<double>("gnss_constraint.min_align_distance", 2.0);
  try_declare.template operator()<bool>("gnss_constraint.use_z", false);
  try_declare.template operator()<bool>("gnss_constraint.use_fixed_transform", false);
  try_declare.template operator()<bool>("gnss_constraint.fixed_anchor_at_start", true);
  try_declare.template operator()<double>("gnss_constraint.fixed_yaw_deg", 0.0);
  try_declare.template operator()<double>("gnss_constraint.fixed_translation_x", 0.0);
  try_declare.template operator()<double>("gnss_constraint.fixed_translation_y", 0.0);
  try_declare.template operator()<double>("gnss_constraint.fixed_translation_z", 0.0);

  try_declare.template operator()<bool>("wheel_constraint.enable", false);
  try_declare.template operator()<std::string>("wheel_constraint.odom_topic", "/wheel/odom");
  try_declare.template operator()<double>("wheel_constraint.max_age", 0.2);
  try_declare.template operator()<double>("wheel_constraint.gain", 0.1);
  try_declare.template operator()<double>("wheel_constraint.max_velocity_correction", 0.1);
  try_declare.template operator()<double>("wheel_constraint.max_speed", 3.0);
  try_declare.template operator()<bool>("wheel_constraint.zero_lateral", true);
  try_declare.template operator()<bool>("wheel_constraint.zero_vertical", true);
  ...
  this->node->get_parameter("gnss_constraint.enable", gnss_constraint_en);
  this->node->get_parameter("gnss_constraint.odom_topic", gnss_odom_topic);
  this->node->get_parameter("gnss_constraint.max_age", gnss_max_age);
  this->node->get_parameter("gnss_constraint.gain", gnss_gain);
  this->node->get_parameter("gnss_constraint.max_correction", gnss_max_correction);
  this->node->get_parameter("gnss_constraint.reject_threshold", gnss_reject_threshold);
  this->node->get_parameter("gnss_constraint.min_align_distance", gnss_min_align_distance);
  this->node->get_parameter("gnss_constraint.use_z", gnss_use_z);
  this->node->get_parameter("gnss_constraint.use_fixed_transform", gnss_use_fixed_transform);
  this->node->get_parameter("gnss_constraint.fixed_anchor_at_start", gnss_fixed_anchor_at_start);
  this->node->get_parameter("gnss_constraint.fixed_yaw_deg", gnss_fixed_yaw_deg);
  this->node->get_parameter("gnss_constraint.fixed_translation_x", gnss_fixed_tx);
  this->node->get_parameter("gnss_constraint.fixed_translation_y", gnss_fixed_ty);
  this->node->get_parameter("gnss_constraint.fixed_translation_z", gnss_fixed_tz);
  const double gnss_fixed_yaw_rad = gnss_fixed_yaw_deg * M_PI / 180.0;
  gnss_fixed_rot = Eigen::AngleAxisd(gnss_fixed_yaw_rad, Eigen::Vector3d::UnitZ()).toRotationMatrix();
  gnss_fixed_translation << gnss_fixed_tx, gnss_fixed_ty, gnss_fixed_tz;

  this->node->get_parameter("wheel_constraint.enable", wheel_constraint_en);
  this->node->get_parameter("wheel_constraint.odom_topic", wheel_odom_topic);
  this->node->get_parameter("wheel_constraint.max_age", wheel_max_age);
  this->node->get_parameter("wheel_constraint.gain", wheel_gain);
  this->node->get_parameter("wheel_constraint.max_velocity_correction", wheel_max_velocity_correction);
  this->node->get_parameter("wheel_constraint.max_speed", wheel_max_speed);
  this->node->get_parameter("wheel_constraint.zero_lateral", wheel_zero_lateral);
  this->node->get_parameter("wheel_constraint.zero_vertical", wheel_zero_vertical);
...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &node, image_transport::ImageTransport &it_)
{
  ...
  if (gnss_constraint_en)
  {
    sub_gnss_odom = this->node->create_subscription<nav_msgs::msg::Odometry>(
        gnss_odom_topic, 100, std::bind(&LIVMapper::gnss_odom_cbk, this, std::placeholders::_1));
    RCLCPP_INFO(this->node->get_logger(), "GNSS soft constraint enabled: %s", gnss_odom_topic.c_str());
    if (gnss_use_fixed_transform)
    {
      RCLCPP_INFO(this->node->get_logger(),
                  "GNSS fixed planar transform enabled: yaw=%.6f deg, t=(%.3f, %.3f, %.3f), anchor_at_start=%s",
                  gnss_fixed_yaw_deg, gnss_fixed_tx, gnss_fixed_ty, gnss_fixed_tz,
                  gnss_fixed_anchor_at_start ? "true" : "false");
    }
  }
  if (wheel_constraint_en)
  {
    sub_wheel_odom = this->node->create_subscription<nav_msgs::msg::Odometry>(
        wheel_odom_topic, 100, std::bind(&LIVMapper::wheel_odom_cbk, this, std::placeholders::_1));
    RCLCPP_INFO(this->node->get_logger(), "Wheel velocity constraint enabled: %s", wheel_odom_topic.c_str());
  }
...
void LIVMapper::gnss_odom_cbk(const nav_msgs::msg::Odometry::ConstSharedPtr &msg_in)
{
  std::lock_guard<std::mutex> lock(mtx_buffer);
  const auto &p = msg_in->pose.pose.position;
  if (!std::isfinite(p.x) || !std::isfinite(p.y) || !std::isfinite(p.z))
  {
    return;
  }
  latest_gnss_odom = *msg_in;
  gnss_has_measurement = true;
}

void LIVMapper::wheel_odom_cbk(const nav_msgs::msg::Odometry::ConstSharedPtr &msg_in)
{
  const auto &v = msg_in->twist.twist.linear;
  if (!std::isfinite(v.x) || !std::isfinite(v.y) || !std::isfinite(v.z) ||
      std::fabs(v.x) > wheel_max_speed)
  {
    return;
  }

  std::lock_guard<std::mutex> lock(mtx_buffer);
  latest_wheel_odom = *msg_in;
  wheel_has_measurement = true;
}

void LIVMapper::applyGnssPositionConstraint()
{
  if (!gnss_constraint_en || !gnss_has_measurement || !lidar_map_inited)
  {
    return;
  }

  nav_msgs::msg::Odometry gnss_msg;
  {
    std::lock_guard<std::mutex> lock(mtx_buffer);
    gnss_msg = latest_gnss_odom;
  }

  const double gnss_time = stamp2Sec(gnss_msg.header.stamp);
  const double lio_time = LidarMeasures.last_lio_update_time;
  if (!std::isfinite(gnss_time) || std::fabs(lio_time - gnss_time) > gnss_max_age)
  {
    RCLCPP_WARN_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 5000,
                         "GNSS constraint skipped: stale measurement age=%.3fs",
                         std::fabs(lio_time - gnss_time));
    return;
  }

  V3D gnss_pos(gnss_msg.pose.pose.position.x,
               gnss_msg.pose.pose.position.y,
               gnss_msg.pose.pose.position.z);

  V3D target_pos = gnss_fixed_rot * gnss_pos + gnss_fixed_translation;

  if (gnss_use_fixed_transform && gnss_fixed_anchor_at_start)
  {
    if (!gnss_reference_inited)
    {
      gnss_reference_pos = gnss_pos;
      livo_reference_pos = _state.pos_end;
      gnss_reference_inited = true;
      gnss_yaw_aligned = true;
      RCLCPP_INFO(this->node->get_logger(),
                  "GNSS fixed-yaw reference initialized: livo=(%.3f, %.3f, %.3f), gnss=(%.3f, %.3f, %.3f)",
                  livo_reference_pos.x(), livo_reference_pos.y(), livo_reference_pos.z(),
                  gnss_reference_pos.x(), gnss_reference_pos.y(), gnss_reference_pos.z());
      return;
    }
    target_pos = livo_reference_pos + gnss_fixed_rot * (gnss_pos - gnss_reference_pos);
  }
  else if (!gnss_use_fixed_transform)
  {
    if (!gnss_reference_inited)
    {
      gnss_reference_pos = gnss_pos;
      livo_reference_pos = _state.pos_end;
      gnss_to_livo_rot = M3D::Identity();
      gnss_reference_inited = true;
      gnss_yaw_aligned = false;
      RCLCPP_INFO(this->node->get_logger(),
                  "GNSS constraint reference initialized: livo=(%.3f, %.3f, %.3f), gnss=(%.3f, %.3f, %.3f)",
                  livo_reference_pos.x(), livo_reference_pos.y(), livo_reference_pos.z(),
                  gnss_reference_pos.x(), gnss_reference_pos.y(), gnss_reference_pos.z());
      return;
    }

    V3D gnss_delta = gnss_pos - gnss_reference_pos;
    V3D livo_delta = _state.pos_end - livo_reference_pos;
    gnss_delta.z() = 0.0;
    livo_delta.z() = 0.0;

    if (!gnss_yaw_aligned)
    {
      const double gnss_dist = gnss_delta.norm();
      const double livo_dist = livo_delta.norm();
      if (gnss_dist < gnss_min_align_distance || livo_dist < gnss_min_align_distance)
      {
        RCLCPP_INFO_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 5000,
                             "GNSS constraint waiting for yaw alignment: gnss=%.2fm livo=%.2fm",
                             gnss_dist, livo_dist);
        return;
      }

      const double gnss_yaw = std::atan2(gnss_delta.y(), gnss_delta.x());
      const double livo_yaw = std::atan2(livo_delta.y(), livo_delta.x());
      const double yaw = livo_yaw - gnss_yaw;
      gnss_to_livo_rot =
          Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ()).toRotationMatrix();
      gnss_yaw_aligned = true;
      RCLCPP_INFO(this->node->get_logger(),
                  "GNSS constraint yaw aligned: yaw=%.2f deg, gnss_dist=%.2fm, livo_dist=%.2fm",
                  yaw * 57.29577951308232, gnss_dist, livo_dist);
    }

    target_pos = livo_reference_pos + gnss_to_livo_rot * (gnss_pos - gnss_reference_pos);
  }
  V3D residual = target_pos - _state.pos_end;
  if (!gnss_use_z)
  {
    residual.z() = 0.0;
  }
  ...
  if (residual_norm > gnss_reject_threshold)
  {
    RCLCPP_WARN_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 5000,
                         "GNSS constraint rejected: residual=%.3fm", residual_norm);
    return;
  }
  ...
  double step_scale = std::clamp(gnss_gain, 0.0, 1.0);
  if (residual_norm * step_scale > gnss_max_correction)
  {
    step_scale = gnss_max_correction / residual_norm;
  }

  const V3D correction = residual * step_scale;
  _state.pos_end += correction;
  _state.cov.block<3, 3>(3, 3) *= 0.95;

  RCLCPP_DEBUG_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 2000,
                        "GNSS constraint applied: residual=%.3fm correction=%.3fm",
                        residual_norm, correction.norm());
}

void LIVMapper::applyWheelVelocityConstraint()
{
  if (!wheel_constraint_en || !wheel_has_measurement || !lidar_map_inited)
  {
    return;
  }

  nav_msgs::msg::Odometry wheel_msg;
  {
    std::lock_guard<std::mutex> lock(mtx_buffer);
    wheel_msg = latest_wheel_odom;
  }

  const double wheel_time = stamp2Sec(wheel_msg.header.stamp);
  const double lio_time = LidarMeasures.last_lio_update_time;
  if (!std::isfinite(wheel_time) || std::fabs(lio_time - wheel_time) > wheel_max_age)
  {
    RCLCPP_WARN_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 5000,
                         "Wheel constraint skipped: stale measurement age=%.3fs",
                         std::fabs(lio_time - wheel_time));
    return;
  }

  const V3D velocity_body = _state.rot_end.transpose() * _state.vel_end;
  V3D target_body = velocity_body;
  target_body.x() = wheel_msg.twist.twist.linear.x;
  if (wheel_zero_lateral)
  {
    target_body.y() = 0.0;
  }
  if (wheel_zero_vertical)
  {
    target_body.z() = 0.0;
  }

  V3D residual_body = target_body - velocity_body;
  const double residual_norm = residual_body.norm();
  if (residual_norm < 1e-6)
  {
    return;
  }

  double step_scale = std::clamp(wheel_gain, 0.0, 1.0);
  if (residual_norm * step_scale > wheel_max_velocity_correction)
  {
    step_scale = wheel_max_velocity_correction / residual_norm;
  }

  const V3D correction_world = _state.rot_end * (residual_body * step_scale);
  _state.vel_end += correction_world;
  _state.cov.block<3, 3>(7, 7) *= 0.95;

  RCLCPP_DEBUG_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 2000,
                        "Wheel constraint applied: residual=%.3fm/s correction=%.3fm/s",
                        residual_norm, correction_world.norm());
}
...
{    
  ...
  applyWheelVelocityConstraint();
  applyGnssPositionConstraint();
  voxelmap_manager->state_ = _state;
...
void LIVMapper::livox_pcl_cbk(const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg_in)
{
  ...
  if (abs(last_timestamp_imu - stamp2Sec(msg->header.stamp)) > 1.0 && !imu_buffer.empty())
  {
    ...
    RCLCPP_INFO_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 5000, "\033[95mSelf sync IMU and LiDAR, HARD time lag is %.10lf \n\033[0m", timediff_imu_wrt_lidar - 0.100);
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
  ...
  {
    RCLCPP_WARN_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 5000, "IMU and LiDAR not synced! delta time: %lf .\n", last_timestamp_lidar - timestamp);
  }
  ...
  {
    RCLCPP_WARN_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 5000, "imu time stamp Jumps %0.4lf seconds \n", timestamp - last_timestamp_imu);
    mtx_buffer.unlock();
    sig_buffer.notify_all();
    return;
  }
...
void LIVMapper::img_cbk(const sensor_msgs::msg::Image::ConstSharedPtr &msg_in)
{
  ...
  {
    RCLCPP_WARN_THROTTLE(this->node->get_logger(), *this->node->get_clock(), 5000, "Image need Jumps: %.6f", img_time_correct);
    mtx_buffer.unlock();
    sig_buffer.notify_all();
    return;
  }
