...
public:
  ...
  void gnss_odom_cbk(const nav_msgs::msg::Odometry::ConstSharedPtr &msg_in);
  void wheel_odom_cbk(const nav_msgs::msg::Odometry::ConstSharedPtr &msg_in);
  void applyGnssPositionConstraint();
  void applyWheelVelocityConstraint();
  ...
  bool gnss_constraint_en = false, gnss_use_z = false, gnss_reference_inited = false, gnss_yaw_aligned = false, gnss_has_measurement = false;
  bool gnss_use_fixed_transform = false;
  bool gnss_fixed_anchor_at_start = true;
  bool wheel_constraint_en = false, wheel_has_measurement = false, wheel_zero_lateral = true, wheel_zero_vertical = true;
  int pcd_save_interval = -1, pcd_index = 0;
  int pub_scan_num = 1;
  double gnss_max_age = 0.3, gnss_gain = 0.05, gnss_max_correction = 0.05, gnss_reject_threshold = 2.0, gnss_min_align_distance = 2.0;
  double gnss_fixed_yaw_deg = 0.0, gnss_fixed_tx = 0.0, gnss_fixed_ty = 0.0, gnss_fixed_tz = 0.0;
  double wheel_max_age = 0.2, wheel_gain = 0.1, wheel_max_velocity_correction = 0.1, wheel_max_speed = 3.0;
  std::string gnss_odom_topic = "/gnss/odom_gated";
  std::string wheel_odom_topic = "/wheel/odom";
  V3D gnss_reference_pos = V3D::Zero();
  V3D livo_reference_pos = V3D::Zero();
  M3D gnss_to_livo_rot = M3D::Identity();
  V3D gnss_fixed_translation = V3D::Zero();
  M3D gnss_fixed_rot = M3D::Identity();
  nav_msgs::msg::Odometry latest_gnss_odom;
  nav_msgs::msg::Odometry latest_wheel_odom;
  ...
  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr sub_gnss_odom;
